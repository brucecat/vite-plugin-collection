...
const plugin = (_options?: ViteOptions): PluginOption[] => {
  ...
  const resolveSwcHelpersDeps = () => {
    ...
    try {
      ...
      const dir = dirname(file).replace(/([\\/])lib$/, "$1src");
      const files = readdirSync(dir);
      helperList = files.map((file) => `@swc/helpers/src/${file}`);
    } catch (e) {
      console.error(e);
    }
    return helperList;
  ...
  return [
    ...
    // dev时热更新2：热更新核心插件
    {
      ...
      config: (userConfig) => {
        ...
        const optimizeDepsDisabled =
          userOptimizeDepsConfig === true || userOptimizeDepsConfig === "dev";
        const optimizeDeps = !optimizeDepsDisabled
          ? [
              "react",
              `${options.jsxImportSource}/jsx-dev-runtime`,
